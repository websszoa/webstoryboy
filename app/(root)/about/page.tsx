...
export default async function ContactPage({ searchParams }: AboutPageProps) {
  const { tab } = await searchParams;
  const initialTab = steps.find((step) => step.number === tab)?.number;
  ...
  return (
    <>
      ...
      />
      <PageAbout initialTab={initialTab} />
    </>
  );
}
